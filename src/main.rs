...
fn main() {
    ...
    let matches = App::new("EdgeDNS")
        ...
        .arg(Arg::with_name("upstream_servers_str")
            .short("u")
            .long("upstream")
            .value_name("server:port,server:port...")
            .help("Comma-delimited list of upstream servers")
            .use_delimiter(true)
            .default_value("8.8.8.8:53,8.8.4.4:53"))
        ...
        .arg(Arg::with_name("cache_size")
            ...
            .long("cachesize")
            .value_name("count")
            .help("Max number of cached entries")
            .takes_value(true)
            .default_value("250000"))
        .arg(Arg::with_name("decrement_ttl")
            .short("r")
            .long("resolver-mode")
            .help("Resolver mode, decrements TTL")
            .takes_value(false))
        .arg(Arg::with_name("failover")
            .short("f")
            .long("failover")
            .help("Failover; try upstream servers sequentially instead of balancing the load")
            .takes_value(false))
        .arg(Arg::with_name("ports_count")
            .short("p")
            .long("ports-count")
            .value_name("count")
            .help("Max number of UDP ports to use for outgoing connections, up to 64511")
            .takes_value(true)
            .default_value("8"))
        .arg(Arg::with_name("upstream_max_failures")
            .short("F")
            .long("max-failures")
            .value_name("count")
            .help("Max failures before marking a server as temporarily unresponsive")
            .takes_value(true)
            .default_value("3"))
        .get_matches();
    let upstream_servers_str: Vec<&str> =
        matches.values_of("upstream_servers_str").unwrap().collect();
    let listen_addr = matches.value_of("listen_addr").unwrap();
    let cache_size = FromStr::from_str(matches.value_of("cache_size").unwrap())
        .expect("Invalid number of cached entries");
    let decrement_ttl = matches.is_present("decrement_ttl");
    let failover = matches.is_present("failover");
    let ports = FromStr::from_str(matches.value_of("ports_count").unwrap())
        .expect("Invalid number of ports");
    let upstream_max_failures = FromStr::from_str(matches.value_of("upstream_max_failures")
            .unwrap())
        .expect("Invalid value for the maximum number of failures");

    let _ = RPDNS::new(cache_size,
                       listen_addr,
                       upstream_servers_str,
                       decrement_ttl,
                       failover,
                       ports,
                       upstream_max_failures);
}
